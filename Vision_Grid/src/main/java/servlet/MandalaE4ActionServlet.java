...
@WebServlet("/MandalaE4ActionServlet")
public class MandalaE4ActionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // DAOに渡すためのリストを準備
        List<ActionBeans> e4ActionsList = new ArrayList<>();

        // JSPからアクションのキーとテキストを取得し、ActionBeansインスタンスを作成
        String[] positions = {"A", "B", "C", "D", "E", "F", "G", "H"};
        for (String position : positions) {
            String actionKey = request.getParameter("e4" + position + "Key");
            String actionText = request.getParameter("e4" + position + "Text");

            // アクションキーが有効かどうかを確認
            if (actionKey != null && !actionKey.isEmpty()) {
                String fullPosition = "e4_" + position;
                // actionTextがnullまたは空の場合も許容
                ActionBeans action = new ActionBeans(
                        elementIdsList.get(3), 
                        fullPosition, 
                        actionKey, 
                        actionText != null ? actionText : ""
                );
                e4ActionsList.add(action);
            }
        }

        // アクションリストが空の場合、エラーメッセージを設定して元のページに戻る
        if (e4ActionsList.isEmpty()) {
            request.setAttribute("errorMessage", "E4に対するアクションが入力されていません。");
        ...
        if (!actionsSetLogic.execute(e4ActionsList)) {
            // 保存が失敗した場合
