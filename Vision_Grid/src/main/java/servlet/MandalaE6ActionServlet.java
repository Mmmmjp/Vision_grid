...
@WebServlet("/MandalaE6ActionServlet")
public class MandalaE6ActionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // DAOに渡すためのリストを準備
        List<ActionBeans> e6ActionsList = new ArrayList<>();

        // JSPからアクションのキーとテキストを取得し、ActionBeansインスタンスを作成
        String[] positions = {"A", "B", "C", "D", "E", "F", "G", "H"};
        for (String position : positions) {
            String actionKey = request.getParameter("e6" + position + "Key");
            String actionText = request.getParameter("e6" + position + "Text");

            // アクションキーが有効かどうかを確認
            if (actionKey != null && !actionKey.isEmpty()) {
                String fullPosition = "e6_" + position;
                // actionTextがnullまたは空の場合も許容
                ActionBeans action = new ActionBeans(
                        elementIdsList.get(5), 
                        fullPosition, 
                        actionKey, 
                        actionText != null ? actionText : ""
                );
                e6ActionsList.add(action);
            }
        }

        // アクションリストが空の場合、エラーメッセージを設定して元のページに戻る
        if (e6ActionsList.isEmpty()) {
            request.setAttribute("errorMessage", "E6に対するアクションが入力されていません。");
        ...
        if (!actionsSetLogic.execute(e6ActionsList)) {
            // 保存が失敗した場合
